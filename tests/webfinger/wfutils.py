...
class LinkSubsetOrEqualsToMatcher(BaseMatcher):
    ...
    def describe_to(self, description: str) -> None:
        description.append_text('Links must be the same or a subset')


class MultiDictHasKeyMatchyer(BaseMatcher):
    """
...
def multi_dict_has_key(arg: str) -> MultiDictHasKeyMatchyer :
    return MultiDictHasKeyMatchyer(arg)