...
class FollowTest:
    ...
    @step
    def unfollow(self):
        self.follower_node.make_follow_undo(self.follower_actor_acct_uri, self.leader_actor_acct_uri)


    @step
    def wait_until_actor_is_unfollowed_by_actor(self):
        self.leader_node.wait_until_actor_is_unfollowed_by_actor(self.leader_actor_acct_uri, self.follower_actor_acct_uri)


    @step
    def wait_until_actor_is_unfollowing_actor(self):
        self.follower_node.wait_until_actor_is_unfollowing_actor(self.follower_actor_acct_uri, self.leader_actor_acct_uri)


    @step
    def leader_creates_note_after_unfollow(self):
        self.leader_note_after_unfollow_uri = self.leader_node.make_create_note(self.leader_actor_acct_uri, f"Testing leader_creates_note_after_unfollow {datetime.now()}")
    ...
    def wait_until_timeout(self):
        try :
            self.follower_node.wait_until_actor_has_received_note(self.follower_actor_acct_uri, self.leader_note_after_unfollow_uri, max_wait=5)
