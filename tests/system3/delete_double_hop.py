...
class DeleteDoubleHopTest:
    ...
    def delete_note(self):
        self.node1.delete_object(self.node1_actor_acct_uri, self.note_uri)


    @step
    def wait_until_note_disappeared(self):
        poll_until(lambda: not self.node2.actor_has_received_object(self.node2_actor_acct_uri, self.note_uri))
        poll_until(lambda: not self.node3.actor_has_received_object(self.node3_actor_acct_uri, self.note_uri))
